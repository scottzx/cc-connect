...
export default function SessionChat() {
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 animate-fade-in">
      {/* Header */}
      <div className="flex flex-row items-center justify-between gap-3 pb-4 border-b border-gray-200 dark:border-gray-800 min-w-0">
        <div className="flex flex-wrap items-center gap-3 min-w-0">
          {!isEmbedded && (
            <Link to="/sessions" className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors shrink-0">
              <ArrowLeft size={18} className="text-gray-400" />
            </Link>
          )}
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white break-all leading-tight">{session?.name || id}</h2>
              <StatusBadge status={bridgeStatus} />
            </div>
            <div className="flex flex-wrap items-center gap-2 mt-0.5 text-xs text-gray-500">
              <Badge className="shrink-0">{project}</Badge>
              {session?.platform && <Badge variant="info" className="shrink-0">{session.platform}</Badge>}
              <span className="break-all">{session?.session_key}</span>
            </div>
          </div>
        </div>
